...
public class TestBase {
    ...
    @BeforeMethod
    @Parameters("env") //getting info from multi_runner.xml file
    public void setUpMethod(@Optional String env){ //getting info from multi_xml file
        System.out.println("env ="+env);

        //if env variable is null use default url
        //if not get url based on env
        if(env==null){
            url=ConfigurationReader.get("url");
        }else {
            //env=qa1
            url=ConfigurationReader.get(env+"_url"); //qa1_url-->comes from config.properties
        }
        driver = Driver.get();
